...
import { Package, LayoutGrid, Printer, Plus, Edit, Trash2, Loader2 } from 'lucide-react';
...
const TabButton = ({ id, label, icon: Icon, activeTab, setActiveTab }) => (
  <button onClick={() => setActiveTab(id)} className={`flex items-center gap-2 px-4 py-3 rounded-t-xl font-bold text-sm transition-all ${activeTab === id ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>
    <Icon size={18} /> {label}
  </button>
);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-blue-700"><Plus size={16} /> Nuevo Producto</button>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {products.map(p => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="p-3"><div className="w-10 h-10 bg-slate-100 rounded-lg overflow-hidden">{p.image_url ? <img src={p.image_url} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-xs font-bold text-slate-300">N/A</div>}</div></td>
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-slate-900"><Plus size={16} /> Nueva Mesa</button>
      </div>
